...
template <typename T>
inline void _print(ostream& stream, int* cords, int* shape, int* stride, size_t nDims, T* val, int ind, int& indent) {
    if (ind == nDims) {
...
class Tensor {
    public:
        ...
        Tensor(const Tensor<T>& other) {
            nDims = other.nDims;
            shape = new int[nDims];
            copy(other.shape, other.shape + nDims, shape);
            stride = new int[nDims];
            copy(other.stride, other.stride + nDims, stride);
        ...
        friend ostream& operator<<(ostream& stream, Tensor<T> tensor) {
            if (tensor.nDims == 0) {
                ...
                fill(cords, cords + tensor.nDims, 0);
                int indent = 0;
