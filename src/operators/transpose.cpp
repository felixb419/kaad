...
Node transpose(Graph &rec, Node input, StaticVector<int> perm) {
    ...
    if (perm.empty()) {
        ...
        value_t = input_val.transpose(shape_buff, strides_buff);
        ...
        value_t = input_val.transpose(shape_buff, strides_buff, perm);
    }

    rec.nodes.push_back(std::make_unique<NodeTransp>(input_ptr, value_t.shape,
                                                     value_t.strides));
